...
from TreeSolver.simulation_tools.simulation_utils import node_to_string, get_leaves_of_tree
from TreeSolver.Node import Node
...
def generate_simulated_full_tree(
    ...
    network.add_nodes(node_to_string(current_depth[0]))
    uniq = 1
...
def generate_simulated_experiment_plasticity(
    ...
    network.nodes[current_depth[0]]["meta"] = np.random.choice(sample_list)
    uniq = 1
                ...
                for _ in range(0, 2):
                    ...
                    if np.random.random() < mu:
                        ...
                        if transition_matrix is None:
                            ...
                            temp_sample_list.remove(network.nodes[node]['meta'])
                            trans = np.random.choice(temp_sample_list)
                            network.nodes[child_node]['meta'] = trans
                            n_transitions += 1
                        else:
                            curr_meta = network.nodes[node]['meta']
                        ...
                        network.nodes[child_node]['meta'] = network.nodes[node]['meta']
